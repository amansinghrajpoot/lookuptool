...
rm ${source_file}_output.csv 2 >/dev/null      #removing any pre-existing output file
...
cn=`( ksh  m_expand -n $lookup_file ) | cat`


if [ $cn -gt 1 ]
then
echo "+Number of partitiions >>> $cn"
( ksh m_cat $lookup_file) | cat  > lookup_temp

lookup_file=lookup_temp
else

echo "+Number of partitions >>> $cn"
 cat $lookup_file  > lookup_temp
 lookup_file=lookup_temp
fi
